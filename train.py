...
def get_model(model_name, weight_path=""):
    ...
    if model_name == "t5-base" or model_name == "t5-small" or model_name == "t5-large":
        ...
        model_name == "gpt2"
        or model_name == "gpt2-medium"
        or model_name == "gpt2-large"
    ):
        model = GPT2Model.from_pretrained(model_path)

    elif (
        model_name == "mt5-base"
        or model_name == "mt5-small"
        or model_name == "mt5-large"
        or model_name == "mt5-xl"
        or model_name == "mt5-xxl"
    ):
        model = MT5ForConditionalGeneration.from_pretrained(model_path)

    elif (
        model_name == "mbart-large-50"
        or model_name == "mbart-large-50-one-to-many-mmt"
        or model_name == "mbart-large-50-many-to-many-mmt"
        or model_name == "mbart-large-50-many-to-one-mmt"
        or model_name == "mbart-large-cc25"
    ):
...
def get_tokenizer(model_name, weight_path=""):
    ...
    if model_name == "t5-base" or model_name == "t5-small" or model_name == "t5-large":
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            model_path, do_lower_case=False, use_fast=False, keep_accents=True
        )
...
class MWPDataSet(Dataset):
    def __init__(
        ...
        assert (
            type_path in valid_type_paths
        ), f"Type path must be one of {valid_type_paths}"
    ...
    def _build(self):
        ...
        with open(source_path, "r") as f_source, open(target_path, "r") as f_target:
            ...
            log.warning(
                f"Using max_src_len, max_tgt_len = ({self.max_src_len}, {self.max_tgt_len})"
            )
...
def get_dataloaders(
    ...
    train_data_set = MWPDataSet(
        tokenizer,
        type_path="train",
        data_dir=data_dir,
        max_examples=num_train,
        max_src_len=k_max_src_len,
        max_tgt_len=k_max_tgt_len,
    )
    eval_data_set = MWPDataSet(
        tokenizer,
        type_path="val",
        data_dir=data_dir,
        max_examples=num_val,
        max_src_len=k_max_src_len,
        max_tgt_len=k_max_tgt_len,
    )
    test_data_set = MWPDataSet(
        tokenizer,
        type_path="test",
        data_dir=data_dir,
        max_examples=num_val,
    ...
    log.info(
        f"Datasets loaded with sizes: train: {len(train_data_set)}, dev: {len(eval_data_set)}, test: {len(test_data_set)}"
    )
...
def forward(model, device, batch):
    ...
    out_dict = model(
        src_ids, attention_mask=src_mask, labels=label_ids, return_dict=True
    )
...
def main():
    ...
    parser.add_argument(
        "--data_dir", type=str, help="Directory of the dataset" "data/", default="data/"
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    parser.add_argument(
        "--batch_size", type=int, help="Batch size. Default: {}".format(4), default=4
    )
    parser.add_argument(
        "--num_workers", type=int, help="num_workers. Default: {}".format(4), default=4
    )

    parser.add_argument(
        "--use_wandb",
        action="store_true",
        help="Whether to log to wandb" "(you'll need to set up wandb env info)",
        default=False,
    )
    ...
    parser.add_argument(
        "--weight_dir",
        type=str,
        help="Directory of the dataset" "data/",
        default="data/",
    )
    parser.add_argument(
        "--use_trained",
    ...
    experiment_name = "{}|{}|{}|{}_{}|{} seed|{}-experiment".format(
        ...
        "multi",
        args.mwp_type,
        int(args.train_split * 10),
        int(args.test_split * 10),
        args.seed_len,
        args.experiment,
    )
    ...
    if args.use_trained:
        ...
        tokenizer = get_tokenizer(args.model, weight_path)
    else:
    ...
    train_loader, dev_loader, test_loader = get_dataloaders(
        ...
        k_max_src_len=args.max_src_len,
    ...
    total_steps = (
        num_train // args.batch_size
    ) * args.epochs  # num times that optim.step() will be called
    ...
    while epoch < args.epochs:
        ...
        if epoch == args.epochs:
            ...
            with torch.no_grad(), tqdm(total=num_val) as progress_bar:
                for batch_num, batch in enumerate(dev_loader):
                    ...
                    loss, _ = forward(
                        model, device, batch
                    )  # loss, logits, but don't need logits
                    loss_meter.update(
                        loss.item(), batch_size
                    )  # loss.item() since it's a tensor
                    ...
                    generated_ids = model.generate(
                        ...
                        max_length=None,
                        min_length=None,
                    )  # (batch x seq length) -------Added min length

                    # collect some stats
                    (
                        total_matches_no_eos,
                        total_matches_with_eos,
                        correct_indices,
                    ) = util.masked_token_match(
                        tgt_ids, generated_ids, return_indices=True
                    )
                    ...
                    orig_text_input, orig_text_output = (
                        batch["source_text"],
                        batch["target_text"],
                    )
                    ...
                    preds = list(
                        zip(orig_text_input, orig_text_output, outputs_decoded)
                    )
                    ...
                    ):  # tensor to list; these are the valid indices
                        pred_list_correct.append(
                            preds[idx[0]]
                        )  # each item was a list of one element

                    # print one batch of generations for qualitative assessment
                    if batch_num == 0:
                        for orig_input, orig_target, actual_output in preds[:1]:
                            log.info(
                                f"Source: {orig_input}\t Target: {orig_target}\n"
            ...
            util.save_preds(
                pred_list_correct, record_dir, file_name="preds_correct_eval.csv"
            )
            results_list = [
                ("NLL", loss_meter.avg),
            ...
            write_output_to_text(
                prediction_list_for_write_to_text, save_predicted_eval_output_text_dir
            )
        ...
        if epoch == args.epochs and args.get_testing_results == True:
            ...
            # save predictions for qualititative analysis
            util.save_preds(
                test_pred_list_all, record_dir, file_name="preds_all_test.csv"
            )
            util.save_preds(
                test_pred_list_correct, record_dir, file_name="preds_correct_test.csv"
            )
            results_list = [
                ("NLL", loss_meter.avg),
                ("exact_match_with_eos", total_matches_with_eos_ct),
            ...
            write_output_to_text(
                Test_prediction_list_for_write_to_text,
                save_predicted_test_output_text_dir,
            )
