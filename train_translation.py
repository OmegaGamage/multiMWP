...
def get_tokenizer(model_name, language = 'en', weight_path = ""):
    ...
    if(model_name =="t5-base" or model_name =="t5-small"  or model_name =="t5-large" ):
        ...
        tokenizer = MBart50Tokenizer.from_pretrained(model_path, src_lang=tokenizer_lang_dic["en"], tgt_lang=language_label)
...
# add a collate function to handle different sizes. Depending on your inputs you should change these.
    ...
    save_dir = os.path.join(args.work_dir, "save_translation", experiment_name)
    save_weights_dir = os.path.join(args.work_dir, "save_weights", experiment_name, "translation")
    ...
    data_dir = os.path.join(args.data_dir,  language, args.experiment, str(args.seed_len), "translation")
    ...
    torch.save(model.state_dict(), save_weights_dir + '/mwptrans_{}.pytorch'.format(args.model))
